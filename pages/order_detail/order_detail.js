...
Page({
  ...
  set_status: function(orderstate) {
    var state_text = '';
    var next_text = '';
    var hiddevaluate = true;
    var hidden_canel = "flex"
    var time_text = '请在约定时间( ' + this.data.send_time+' )内送达'
    switch (orderstate) {
      case 0:
        state_text = "等待用户支付",
        hidden_canel = "none"
      ...
      case 6:
        ...
        if (canceltype == 11) {
          var time_text = '用户已取消订单，如有疑问请咨询客服';
        } else if (canceltype == 12) {
          var time_text = '你已取消订单，请再接再厉';
        }
        break;
    }
    this.setData({
  ...
  make_operation: function() {
    ...
    if (orderstate == 1) {
      ...
      wx.request({
        url: app.globalData.URL + '/order/runner/confirm.do?id=' + o_id,
        method: 'get',
        dataType: 'json',
        header: {
          'Cookie': wx.getStorageSync('cookieKey')
  ...
  canel_order: function() {
    ...
    wx.showModal({
      ...
      success: function(res) {
        ...
        if (res.confirm) {
          wx.request({
            url: app.globalData.URL + '/order/runner/cancel.do?id=' + o_id,
            method: 'get',
            dataType: 'json',
            header: {
              'Cookie': wx.getStorageSync('cookieKey')
